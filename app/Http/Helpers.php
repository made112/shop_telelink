...
if (!function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        } elseif (OtpConfiguration::where('type', 'bulksms')->first()->value == 1) {
            $token = env('BULKSMS_API_TOKEN');
            if (str_contains($to, '+972')) {
                $to = '0' . explode('+972', $to)[1];
            } else $to = explode('+97', $to)[1];
            $url = env("BULKSMS_URL") . '/?apikey=' . $token . '&to=' . $to . '&msg=' . $text;
            try {
                if (strlen($to) > 10) {
                    $to = substr($to, '3');
                } elseif (strlen($to) < 10) {
...
//                OTP Logger
                log_otp($url, $res->getBody());

                return $res->getBody();
            } catch (\Exception $e) {
                log_otp($url, $e->getMessage());
                return $e->getMessage();
            }
...
if (!function_exists('log_otp')) {
    function log_otp($req, $res)
    {
        $otp_log = new \App\OTPLogger;
        $otp_log->request_content = $req;
        $otp_log->response_content = $res;
        $otp_log->save();
    }
}
...
if (!function_exists('check_text_ar')) {
    function check_text_ar($data)
    {
        if (preg_match('/^[^a-zA-Z0-9]+$/', $data)) // '/[^a-z\d]/i' should also work.
        {
            $data = str_replace("\"", "", json_encode(strtolower($data)));
            $pos = strpos($data, '\\');
            $str = '';
            if ($pos !== false) {
                $str = substr($data, 0, $pos + 1) . str_replace('\\', '\\\\', substr($data, $pos + 1));
            }
            $data = $str;
        }
        return $data;
...
// Get role from Route Current Controller
if (!function_exists('get_role_controller')) {
    function get_role_controller($name)
    {
        $all_name_contr = array(
            'ProductController' => 1,
            'CategoryController' => 1,
            'BrandController' => 1,
            'SubCategoryController' => 1,
            'SubSubCategoryController' => 1,
            'ReviewController' => 1,
            'FlashDealController' => 2,
            'SponsoredProductController' => 16,
            'PickupPointController' => 14,
            'RefundRequestController' => 17,
            'SellerController' => 5,
            'CustomerController' => 6,
            'BusinessSettingsController' => 8,
            'ConversationController' => 18,
            'BisanController' => 19,
            'MessageController' => 7,
            'ReportController' => 20,
            'HomeController' => 9,
            'AttributeController' => 12,
            'CouponController' => 12,
            'CountryController' => 12,
            'CityController' => 12,
            'ManualPaymentMethodController' => 21,
            'WalletController' => 21,
            'ClubPointController' => 22,
            'OTPController' => 23,
            'SupportTicketController' => 13,
            'SEOController' => 11,
            'RoleController' => 10,
            'StaffController' => 10
        );
        if (array_key_exists($name, $all_name_contr)) {
            return $all_name_contr[$name];
        }
    }
}
...
// Get City Name
if (!function_exists('getNameCity')) {
    function getNameCity($id)
    {
        $city = \App\City::find(intval($id));
        if (isset($city)) {
            return $city->name;
        }
        return '';
...
//        return $file."?v=$timestamp";
        ...
        return $file . "?v=" . config('app.version');
    }
}
...
// Take number of point for sellers and convert it to level
// Bronze, Silver, Gold, Diamond
if (!function_exists('convert_points_to_levels')) {
    function convert_points_to_levels($points)
    {
        if ($points == 0) {
            return 'Basic';
        }
        $bronze_level = BusinessSetting::where('type', 'BRONZE_LEVEL')->first();
        $silver_level = BusinessSetting::where('type', 'SILVER_LEVEL')->first();
        $gold_level = BusinessSetting::where('type', 'GOLD_LEVEL')->first();
        $diamond_level = BusinessSetting::where('type', 'DIAMOND_LEVEL')->first();

        if ($points >= $bronze_level->value and $points < $silver_level->value) {
            return 'Bronze';
        } elseif ($points >= $silver_level->value and $points < $gold_level->value) {
            return 'Silver';
        } elseif ($points >= $gold_level->value and $points < $diamond_level->value) {
            return 'Gold';
        } elseif ($points >= $diamond_level->value) {
            return 'Diamond';
        } else {
            return 'Basic';
        }
    ...
    function home_price($id)
    {
        ...
        if ($product->variant_product) {
            foreach ($product->stocks as $key => $stock) {
                $lowest_price = $stock->getOriginal('price');
                $highest_price = $stock->getOriginal('price');
                if ($lowest_price > $stock->price) {
                    $lowest_price = $stock->price;
                }
                if ($highest_price < $stock->getOriginal('price')) {
                    $highest_price = $stock->getOriginal('price');
                }
            }
        }
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($id)
    {
        ...
        if ($product_stock = \App\ProductStock::where('product_id', $product->id)->first()) {
    ...
    if (Session::get('cart') && count(Session::get('cart')) > 0) {
        ...
        foreach (Session::get('cart') as $key => $cartItem) {
            ...
            if ($product->added_by == 'admin') {
                $admin_products = true;
            } elseif (!in_array($product->user_id, $ids_user)) {
                array_push($ids_user, $product->user_id);
            }
        }
        $city = \App\City::find(request()->session()->get('shipping_info')['city']);
        if ($city && $city->type != 'occupied_interior') {
        ...
        if ($admin_products) {
            $total_shipping += intval(BusinessSetting::where('type', $city->type)->first()->value);
        }
        if (count($ids_user) > 0) {
            if ($city) {
                if ($subtotal < $free_shipping) {
                    foreach ($ids_user as $id) {
                        if ($city->type == 'occupied_interior') {
                            $total_shipping += \App\Shop::where('user_id', $id)->first()->shipping_cost_oi;
                        } elseif ($city->type == 'west_bank') {
                            $total_shipping += \App\Shop::where('user_id', $id)->first()->shipping_cost_wb;
                        } elseif ($city->type == 'jerusalem') {
                            $total_shipping += \App\Shop::where('user_id', $id)->first()->shipping_cost_j;
                        } else {
                        }
                    }
                } else {
        ...
        return $total_shipping / count(Session::get('cart'));
    }
    return 0;
}

function getCollectiveDelivery()
{
    return getDirectDelivery() / 2;
}
        ...
        if (Session::get('cart') && count(Session::get('cart')) > 0) {
            foreach (Session::get('cart') as $key => $cartItem) {
                ...
                if ($product->added_by == 'admin') {
                    ...
                    $flag_seller = true;
                    if (count($ids_sellers) > 0) {
                        if (in_array($product->user_id, $ids_sellers)) {
                            array_push($ids_sellers, $product->user_id);
                        } else {
                            return false;
                        }
                    } else {
            ...
            if (count($items) == count(Session::get('cart'))) {
                if ($city && $city->type != 'occupied_interior') {
                    if (intval($subtotal) >= $free_shipping) {
                        $calculate_shipping = 0;
                    } else {
                        $calculate_shipping = intval(BusinessSetting::where('type', $city->type)->first()->value);
                    }
                } else {
                    $calculate_shipping = intval(BusinessSetting::where('type', $city->type)->first()->value);
                }
            }
            if (count($ids_sellers) == count(Session::get('cart'))) {
                if ($city) {
                    foreach ($ids_sellers as $id) {
                        $seller_shop = \App\Shop::where('user_id', $id)->first();
                        if (isset($seller_shop->deal_with) && $seller_shop->deal_with == 0) {
                            $total_products_for_seller = getTotalSellerPorductsInOrder($seller_shop->user_id);
                            if (round($total_products_for_seller) < $seller_shop->shipping_free) {
                                if ($city->type == 'occupied_interior') {
                                    $calculate_shipping = $seller_shop->shipping_cost_oi;
                                } elseif ($city->type == 'west_bank') {
                                    $calculate_shipping = $seller_shop->shipping_cost_wb;
                                } elseif ($city->type == 'jerusalem') {
                                    $calculate_shipping = $seller_shop->shipping_cost_j;
                                }
                            } else {
                                if ($city->type != 'occupied_interior') {
                                    $calculate_shipping = 0;
                                } else $calculate_shipping = $seller_shop->shipping_cost_oi;
                            }
                        } else {
                            dd('Deal With iBuy');
                        }
...
//                return \App\BusinessSetting::where('type', 'shipping_cost_admin')->first()->value/count($admin_products);
                return $calculate_shipping / count($items);
            } else {
//                dd($calculate_shipping / count($ids_sellers));
//                return \App\Shop::where('user_id', $product->user_id)->first()->shipping_cost/count($seller_products[$product->user_id]);
                return $calculate_shipping / count($ids_sellers);
            }
        } else {
            return $calculate_shipping / count(Session::get('cart'));
...
function getTotalSellerPorductsInOrder($id)
{
    if (Session::get('cart') && count(Session::get('cart')) > 0) {
        $subtotal = 0;
        foreach (Session::get('cart') as $key => $cartItem) {
            if (Product::find($cartItem['id'])->user_id == $id) {
                $subtotal += $cartItem['price'] * $cartItem['quantity'];
            }
        }
...
if (!function_exists('isProductsDigit')) {
    function isProductDigit()
    {
        $digital_status = 0;
        if (Session::get('cart') && count(Session::get('cart')) > 0) {
            foreach (Session::get('cart') as $key => $cartItem) {
...
if (!function_exists('isOrderDetailssDigit')) {
    function isOrderDetailssDigit($orderDetails)
    {
        $digital_status = 0;
...
if (!function_exists('getTotalFromCart')) {
