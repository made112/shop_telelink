...
class Product extends Model
{
    ...
    public $translatable = ['name', 'unit', 'description', 'tags', 'meta_title', 'meta_description'];
    ...
    public function getThumbnailImgAttribute($value) {
        if(is_numeric($value)){
            ...
            $file_path = 'thumbnails/'.explode('/',uploaded_asset($tmp))[2];
    ...
    public function getPhotosAttribute($value) {
        ...
        if(is_string($value)){
            ...
            foreach ($origin_arr as $item) {
                $file_path = 'thumbnails/'.explode('/',uploaded_asset($item))[2];
                if (file_exists(public_path($file_path))) {
                    array_push($tmp_arr, 'thumbnails/'.explode('/',uploaded_asset($item))[2]);
                } else {
