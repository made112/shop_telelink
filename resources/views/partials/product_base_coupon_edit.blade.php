...
<div class="form-group">
    ...
    <div class="col-lg-9">
        <input type="number" placeholder="{{translate('Coupon Usage')}}" id="usage" name="usage" value="{{ $coupon->usage }}" class="form-control" required>
    </div>
</div>
                ...
                <div class="col-lg-9">
                    <select class="form-control @error('users') is-invalid @enderror demo-select2" name="users[]" multiple data-selected-text-format="count" data-actions-box="true">
                        @foreach($users as $user)
                            <option
                                value="{{$user->id}}"
                                @if (isset($details->users) and in_array($user->id, $details->users)) selected @endif>
                                {{$user->email}}
                            </option>
                        @endforeach
                    </select>
