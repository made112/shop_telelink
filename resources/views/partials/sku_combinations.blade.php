@if(count($combinations[0]) > 0)
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <td class="text-center">
                <label for="" class="control-label">{{translate('Price After Discount')}}</label>
            </td>
            <td class="text-center">
        ...
        @foreach ($combinations as $key => $combination)
            ...
            @if(strlen($str) > 0)
                <tr>
                    ...
                    <td>
                        <input type="number" name="discount_{{ $str }}" value="{{$discount}}" min="0" step="0.01"
                               class="form-control" required>
                    </td>
                    <td>
                        <input type="text" name="sku_{{ $str }}" value="{{ $sku }}" class="form-control"
                               @if(Auth::user()->user_type === 'admin' || Auth::user()->user_type === 'staff') required @endif>
                    </td>
                    <td>
                        <input type="number" name="qty_{{ $str }}" value="10" min="0" step="1" class="form-control"
                               required>
                    </td>
                </tr>
            @endif
