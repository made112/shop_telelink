            ...
            <div class="row">
                ...
                <div class="pt-4 col-12 mb-5">
                    <ul class="process-steps clearfix">
                        ...
                        <li @if($delivery_status == 'on_review') class="active" @elseif($delivery_status == 'on_delivery' || $delivery_status == 'delivered') class="done" @endif>
                            <div class="icon">2</div>
                            <div class="title">{{ translate('On review')}}</div>
                        </li>
                        <li @if($delivery_status == 'on_delivery') class="active" @elseif($delivery_status == 'delivered') class="done" @endif>
    			...
    			<div class="col-sm-6 text-xs-center">
    				<address>
        				<strong class="text-main">{{ isset(json_decode($order->shipping_address)->name) ? json_decode($order->shipping_address)->name : '' }}</strong><br>
        				 ...
        				 {{ isset(json_decode($order->shipping_address)->address) ? json_decode($order->shipping_address)->address : '' }}, {{ isset(json_decode($order->shipping_address)->city) ? json_decode($order->shipping_address)->city : '' }}, {{ isset(json_decode($order->shipping_address)->country) ? json_decode($order->shipping_address)->country : '' }}
                    </address>
                    @if ($order->manual_payment && is_array(json_decode($order->manual_payment_data, true)))
                        <br>
                        <strong class="text-main">{{ translate('Payment Information') }}</strong><br>
                        {{ translate('Name') }}: {{ isset(json_decode($order->manual_payment_data)->name) ? json_decode($order->manual_payment_data)->name : '' }}, {{ translate('Amount') }}: {{ single_price(json_decode($order->manual_payment_data)->amount) }}, {{ translate('TRX ID') }}: {{ json_decode($order->manual_payment_data)->trx_id }}
                        <br>
