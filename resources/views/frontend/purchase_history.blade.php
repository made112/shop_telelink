                                        ...
                                        @if (count($orders) > 0)
                                            @foreach ($orders as $key => $order)
                                                @if (count($order->orderDetails) > 0)
                                                    ...
                                                    $dateOrder = \Carbon\Carbon::make($order->created_at);
                                                    $dateOrderAfter3 = $dateOrder->addDays(3);
                                                    $check_date = $dateOrder->timestamp >= \Carbon\Carbon::now()->timestamp  && \Carbon\Carbon::now()->timestamp < $dateOrderAfter3->timestamp;
                                                    $status = $orderDetails->delivery_status != 'deleiverd' && $orderDetails->delivery_status != 'cancelled' && $orderDetails->payment_status != 'paid';
                                                    @endphp
                                                    <tr>
                                                        <td>
                                                            ...
                                                            <div class="dropdown">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="">
                                                                    ...
                                                                    @if($check_date && $status)
                                                                        <a onclick="confirm_modal('{{route('orders.destroy', $order->id)}}');" class="dropdown-item confirm-delete" data-href="{{route('orders.destroy', $order->id)}}" title="{{ translate('Cancel Order') }}">
                                        ...
                                        @else
                                            <tr>
                                                <td class="text-center" colspan="6">
                                                    ...
                                                    <span class="h4" style="font-size: 20px; color: #818a91">{{__('There is no purchase orders')}}</span>
                                                </td>
                                            </tr>
                                        @endif
                                        </tbody>
