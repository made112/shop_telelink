                ...
                <div class="box-inline pad-rgt pull-left">
                    <div class="select" style="min-width: 300px;">
                        <select class="form-control demo-select2" name="rating" id="rating" onchange="filter_by_rating()">
                            <option value="no-filter">{{translate('Filter by Rating')}}</option>
                            <option value="rating,desc" @if(isset($rating) and $rating === 'rating,desc') selected @endif >{{translate('Rating (High > Low)')}}</option>
                            <option value="rating,asc" @if(isset($rating) and $rating === 'rating,asc') selected @endif>{{translate('Rating (Low > High)')}}</option>
                        </select>
                    </div>
                </div>
                <div class="box-inline pad-rgt pull-left">
                    <div class="" style="min-width: 200px;">
                        <input type="text" class="form-control" id="search" name="search"@isset($sort_search) value="{{ $sort_search }}" @endisset placeholder="{{ translate('Enter Product Name') }}">
                    </div>
                </div>
            </form>
        </div>
