            ...
            <div class="invoice-bill row">
                <div class="col-sm-6 text-xs-center">
                    <address>
                        ...
                        {{ isset(json_decode($order->shipping_address)->city) ? getNameCity(json_decode($order->shipping_address)->city) : '' }} <br>
                        {{ isset(json_decode($order->shipping_address)->address)? json_decode($order->shipping_address)->address : '' }}, {{ isset(json_decode($order->shipping_address)->postal_code) ? json_decode($order->shipping_address)->postal_code : ''}}<br>
                        {{ isset(json_decode($order->shipping_address)->country) ? json_decode($order->shipping_address)->country : '' }}
                    </address>
                    @if ($order->manual_payment && is_array(json_decode($order->manual_payment_data, true)))
                        <br>
                        <strong class="text-main">{{ translate('Payment Information') }}</strong><br>
                        Name: {{ isset(json_decode($order->manual_payment_data)->name) ? json_decode($order->manual_payment_data)->name : '' }}, TRX
