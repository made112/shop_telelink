...
function exportFile() {
    $('#exportFiles').on('change', function (){
        if($(this).val() != ''){
            $('#formExport').submit();
...
function disableBtnAddFiles(el) {
    $(el).on('shown.bs.tab', function (){
        $('[data-toggle="aizUploaderAddSelected"]').attr('disabled', 'disabled').addClass('disabled');
        $('#txt_msg').text('You must return back to select files');
    })
}
function enableBtnAddFiles(el) {
    $(el).on('shown.bs.tab', function (){
        $('[data-toggle="aizUploaderAddSelected"]').attr('disabled', false).removeClass('disabled');
        $('#txt_msg').text('');
    })
}
